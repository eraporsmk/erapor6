...
class CreateNilaiUsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('nilai_us', function (Blueprint $table) {
            $table->uuid('nilai_us_id');
    ...
    {
        Schema::table('nilai_us', function (Blueprint $table) {
            $table->dropForeign(['anggota_rombel_id']);
			$table->dropForeign(['pembelajaran_id']);
			$table->dropForeign(['sekolah_id']);
        });
        Schema::dropIfExists('nilai_us');
    }
}
