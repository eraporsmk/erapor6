...
class DataAsesor extends Component
{
    ...
    public function render()
    {
        return view('livewire.referensi.data-asesor', [
            ...
            'breadcrumbs' => [
                ['link' => "/", 'name' => "Beranda"], ['link' => '#', 'name' => 'Referensi'], ['name' => "Data Asesor"]
            ],
            'tombol_add' => [
                'wire' => 'addModal',
                'color' => 'primary',
    ...
    public function detil($id){
        ...
        $this->guru_id = $id;
        $this->guru = Guru::with(['gelar_depan', 'gelar_belakang', 'dudi'])->find($id);
        $this->dudi_id = ($this->guru->dudi) ? $this->guru->dudi->dudi_id : NULL;
        ...
        $this->ref_dudi = Dudi::orderBy('nama')->where('sekolah_id', session('sekolah_id'))->get();
        ...
        $this->dispatchBrowserEvent('ref_dudi', ['ref_dudi' => $this->ref_dudi]);
        $this->dispatchBrowserEvent('dudi_id', ['dudi_id' => $this->dudi_id]);
